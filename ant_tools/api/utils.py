...
def barcode_generator(data,show=False):
    """
    This function will accept a parameter 'data' and return an SVG data as a barcode.
    It can be accessed through a Jinja tag as {{ barcode_generator(data) }}.
    """
    try:
        ...
        # Remove text elements using regular expressions
        if show:
                svg_data = re.sub(r'<text[^>]*>.*?</text>', '', svg_data, flags=re.DOTALL)

        # Encode the modified SVG data as base64
